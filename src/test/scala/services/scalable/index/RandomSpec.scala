...
class RandomSpec extends AnyFlatSpec with Repeatable {
  ...
  "it " should "serialize and order datoms (serialized as array of arrays of bytes) correctly " in {
    ...
    var v = rand.nextInt(1, 5) match {
      case 1 => abs(rand.nextInt(0, abs.length)).getV
      case 2 => minAge
      case 3 => ByteString.copyFrom(colors(rand.nextInt(0, colors.length)).getBytes(Charsets.UTF_8))
      case 4 => names(rand.nextInt(0, names.length)).getV
    }

    var vupper = rand.nextInt(1, 5) match {
      case 1 => abs(rand.nextInt(0, abs.length)).getV
      case 2 => maxAge
      case 3 => ByteString.copyFrom(colors(rand.nextInt(0, colors.length)).getBytes(Charsets.UTF_8))
      case 4 => names(rand.nextInt(0, names.length)).getV
    ...
    val r = Await.result(findAll(), Duration.Inf).map { d =>
      d.a -> new String(d.getV.toByteArray) -> d.e
    ...
    def formatDatom(d: Datom): String = {
      ...
      if(d.v.isDefined){
        arr = arr :+ new String(d.getV.toByteArray)
      }
