...
class MainSpec extends Repeatable {
  ...
  "operations" should " run successfully" in {
    ...
    val ctx = new DefaultContext[K, V](indexContext.id, indexContext.root.map{r => (r.partition, r.id)},
    ...
    db = db.withIndexes(Seq(
      IndexContext("test-main-index", NUM_LEAF_ENTRIES, NUM_META_ENTRIES, indexCtx.root.map{r => RootRef(r._1, r._2)}, indexCtx.levels, indexCtx.num_elements)
    ))
