...
object Errors {
  ...
  case class VERSION_CHANGED[K, V](changes: Seq[Tuple3[K, Option[String], Option[String]]], ks: K => String)
    extends RuntimeException(s"changed versions: ${changes.map{case (k, vs0, vs1) => ks(k) -> (vs0, vs1)}}!") with IndexError
